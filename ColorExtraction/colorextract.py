...
def gs_hist(filename):

    ir_img = Image.open(filename).convert('L')
...
def spectrogram(filenames,hsv):

    vis = pl.hist_hsv(filenames[0])
    # vis_bin = np.histogram(
    #     vis[..., hsv].flatten() * 255, bins=256, range=(0.0, 255.0))[0]
    uv = pl.hist_hsv(filenames[1])
    # uv_bin = np.histogram(
    #     uv[..., hsv].flatten() * 255, bins=256, range=(0.0, 255.0))[0]
    ir = pl.hist(filenames[2])

    print vis.shape

    plt.fill_between(vis_range, 0, vis[hsv])
    plt.fill_between(ir_range, 0, ir)
    plt.fill_between(uv_range, 0, uv[hsv])
    plt.show()

colors, sizes, hist_c, hist_s = load_img(
    '../imgs/copenhagen-painting-thumbnail.jpg', hist=True)

# Sort histogram by hue (Sorting both prominence and colors)
hist_h_c, hist_h_s = zip(
    *sorted(zip(hist_c, hist_s), key=get_hsv_h, reverse=True))
...
# These images show the true color reflectogram of this painting and the near infrared reflectogram (1000-1200 nm).
# http://www.artexpertswebsite.com/scientific-tests/ir-photography.php
...
vis_range = np.linspace(390.0, 750.0, 256)
ir_range = np.linspace(1000.0, 1200.0, 256)
uv_range = np.linspace(280.0, 400.0, 256)

rgb_hist('../imgs/copenhagen-painting-thumbnail.jpg')
hsv_hist('../imgs/copenhagen-painting-thumbnail.jpg')
hsv_hist('../imgs/copenhagen-uv-thumbnail.jpg')

files = ['../imgs/copenhagen-painting-thumbnail.jpg','../imgs/copenhagen-uv-thumbnail.jpg','../imgs/copenhagen-infrared-thumbnail.jpg']
